...
void main() {
		...
		tex_color = abs(1 / 3 - 1) * tex_color + 1 / 3 * texture2D(fog, fs_in.TexCoords + fogPos) * tex_color;
    	...
    	FragColor.rgb = vec3(dot(FragColor.rgb, vec3(0.299, 0.587, 0.114))) * rand(vec2(seed) * fs_in.FragPos.xz);
    	FragColor = FragColor * fs_in.shade;
    	if(brightness > 1) {
