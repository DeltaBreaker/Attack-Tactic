...
void main() {
	...
	Normals = vec4(fs_in.Normal, fs_in.shade.w);
	FragPos = vec4(fs_in.FragPos, fs_in.shade.w);
	Material = vec4(ambiance_intensity, diffuse_intensity, specular_intensity, fs_in.shade.w);
	Misc = vec4(shadow, 3 + fs_in.shade.w, shininess, fs_in.shade.w);
