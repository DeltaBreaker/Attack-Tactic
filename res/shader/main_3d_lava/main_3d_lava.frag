...
void main() {
		float pixelCoordY = mod(fs_in.shadePass.x + 3.75, 16.0) / 8.0 + 0.03125;
		float pixelCoordX = mod(fs_in.shadePass.z + 3.75, 16.0) / 8.0 + 0.03125;
		vec4 shade = vec4(texture(lava, vec2(pixelCoordX, pixelCoordY)).rgb, 0.75);
		...
		FragColor = tex_color * shade * vec4(lighting, 1);
