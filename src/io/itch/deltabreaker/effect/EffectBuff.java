...
public class EffectBuff extends Effect {
	...
	private ArrayList<BoostParticle> particles = new ArrayList<>();
	private int amt = 8;
	private int count = 0;
	private Light light;

	public EffectBuff(Vector3f position) {
	...
	public void tick() {
		if (count < amt) {
			count++;
			particles.add(new BoostParticle(Vector3f.add(position, 0, 0, 0)));
		}
...
class BoostParticle {
	...
	public BoostParticle(Vector3f position) {
	...
	public void tick() {
		age++;
		position = Vector3f.add(start, AdvMath.sin[sin % 360] * 8, heightInterval * age, (float) Math.cos(Math.toRadians(sin)) * 8);
