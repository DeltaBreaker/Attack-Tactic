...
public class EffectBattlePierce extends EffectBattle {
	...
	public EffectBattlePierce(Vector3f position, boolean dir) {
		...
		this.dir = dir;
		AudioManager.getSound("battle_bow_" + new Random().nextInt(2) + ".ogg").play(AudioManager.defaultBattleSFXGain, false);
	}
	...
	public void render() {
		if (frame < frames - 1) {
			BatchSorter.add("z", "effect_battle_physical_1_" + frame + ".dae", "effect_battle_physical_1_" + frame + ".png",
					"main_3d", "DEFAULT", Vector3f.add(position, dir ? -1 : 1, -2, 0), rotation, scale,
