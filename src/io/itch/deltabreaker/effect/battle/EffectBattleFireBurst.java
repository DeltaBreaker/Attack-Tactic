...
public class EffectBattleFireBurst extends EffectBattle {
	...
	public EffectBattleFireBurst(Vector3f position, boolean dir) {
		...
		light = new Light(Vector3f.mul(this.position.add(new Vector3f(0, 1, 0)), new Vector3f(0.5f, 0.5f, 0.5f)),
				new Vector3f(3f, 1.25f, 0f), 1f, 0.002f, 0.035f, null);
	...
	public void render() {
		if (frame < frames - 1) {
			BatchSorter.add("effect_battle_fire_2_" + frame + ".dae", "effect_battle_fire_2_" + frame + ".png",
					"main_3d", "DEFAULT", position, rotation, scale, new Vector4f(1f, 1f, 1f, 1), true, false);
		}
		for (int i = 0; i < sparks.size(); i++) {
			sparks.get(i).render();
		}
